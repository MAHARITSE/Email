...
export default function App() {
  ...
  const handleSignInWithToken = async (tokenInput: string) => {
    try {
      setIsLoggingIn(true);
      setAuthError(null);
      const res = await signInWithAccessToken(tokenInput);
      setUser(res.user);
      setToken(res.accessToken);
      setNeedsAuth(false);
      setAccounts(getStoredAccounts());
      showToast(`Connexion réussie : ${res.user.email || res.user.displayName}`);
    } catch (err: any) {
      console.error('Direct token sign-in error:', err);
      setAuthError(err?.message || 'Jeton d\'accès invalide ou expiré.');
      throw err;
    } finally {
      setIsLoggingIn(false);
    }
  ...
  if (needsAuth || !token) {
    return (
      <SignInPrompt
        ...
        onSignInWithToken={handleSignInWithToken}
